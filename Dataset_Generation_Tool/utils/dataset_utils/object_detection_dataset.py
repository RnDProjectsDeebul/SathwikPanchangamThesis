...
from utils.dataset_utils.scene_utils import arrange_objects,place_objects_no_collision
from utils.dataset_utils.annotation_utils import save_annotation_files_object_detection,save_as_json_file,update_json_file,TimerLogger
...
def generate_object_detection_dataset(scene, surface_to_place, object_names, class_to_id, camera, config_file, light_source):
    """
    Function to generate an Object Detection dataset.
    ...
    print("Generating Object Detection Dataset")
    ...
    if "normal" not in test_constraints:
        raise ValueError("Normal constraint is mandatory for Object Detection Dataset Generation")
        ...
        if constraint == 'normal':
            ...
            for idx,value in enumerate(sequential_values):
                ...
                for obj_to_render in object_names:
                    ...
                    # print("Objects location after placing on the surface : ",bpy.data.objects[obj_to_render].location)
                ...
                for image_type in config_file['dataset_settings']['image_types']:
                    render_dataset_images(scene=scene,idx=idx,image_type=image_type,img_file_format=img_file_format,output_path=output_path,object_names=object_names,camera=camera,
                                          links=links,render_layers_node=render_layers_node,output_node=output_node)

                # Store object detection annotations in the respective files
                save_annotation_files_object_detection(idx,scene,camera,
                                                       light_source,deformation_value,
                                                       object_names,class_to_id,
                                                       output_path=output_path
                                                       ,dataset_label_format=dataset_labels_format)
            ...
            for idx in range(num_images):
                for obj_to_render in object_names:
                    ...
                    if object_placement == 'random':
                        for obj_name in object_names:
                            place_object_random_postions(scene,obj=bpy.data.objects[obj_name],
